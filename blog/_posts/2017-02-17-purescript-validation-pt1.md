...
Algebraic Data Types in PureScript, the most well know are `Maybe` and `Either`.
With `Maybe` we can use the `Nothing` data constructor to signal failure.
Unfortunately, any context about the failure is lost because `Nothing` is
...
We could ensure we only constructo a `Person` with validate data by providing
...
Here we're going to use `Maybe` to signal success or failure, but we still
need a function that can run each of these checks, and then construct a `Person`.
...
With `Either` we can provide more information.
...
Since `Either` can provide more information about our failure, we creae a
`PersonError` type to describe what failed at each step. Now if we try to create
a `Person` using invalid inputs we get something better than with `Maybe`
...
The nature of the `Applicative` instances for `Either` means that we can't
...
Enter the `purescript-validation` library, which provides alternative ways to
perform validation.
...
implement the `Semigroup` typeclass. `Data.Validate.Semiring` which I'll cover in
a separate post.
...
the difference is the in the `Applicative` instance which says that any errors
...
That's a lot more change that we saw between `Maybe` and `Either`, so let's unpack
it a bit.
